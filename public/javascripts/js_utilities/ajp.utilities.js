/*global AJP*/ // Used by JSLint to exclude CSD from search of undefined variables and functions.
 
	//remember to make this file read only in the submodules you make of it
...
//	})();
	...
	//prevents the &nbsp;  unicode character (160 ) from mucking up the indexOf function.
	String.method('replace_nbsp', function () {
		return this.replace(/\u00a0/g, ' ');
	});
	
	//prevents the &nbsp;  unicode character (160 ) from mucking up the indexOf function.
	String.method('indexOf_v2', function (another_string) {
		return this.replace_nbsp().indexOf(another_string.replace_nbsp());
	});
	
	//prevents the &nbsp;  unicode character (160 ) from mucking up the indexOf function.
	String.method('lastIndexOf_v2', function (another_string) {
		return this.replace_nbsp().lastIndexOf(another_string.replace_nbsp());
	});
	...
	//// add the some method to arrays, as described in https://developer.mozilla.org/en/JavaScript/Reference/Global_Objects/Array/some
	...
	//// method returns an array where all of the values in another_array are removed from the original array.
	Array.method('remove_array', function (another_array) {
		var i = 0, len = another_array.length;
		var copy_of_this_array = [].concat(this);
		
		for (i=0; i < len; i += 1) {
			//for each element in the 'value' array, remove it from 'this' array
			copy_of_this_array = copy_of_this_array.remove(another_array[i]);
		};
		return copy_of_this_array;
	});
	...
	Array.method('unique', function () {
		var is_contained_once = function(element, index, array) {
			...
			return (sub_array.does_not_contain(element));
		};

		return this.filter(is_contained_once);
	});
...
//		});
	...
	AJP.u.safely_add(AJP.u, 'get_text_nodes_in', function (node, includeWhitespaceNodes) {
