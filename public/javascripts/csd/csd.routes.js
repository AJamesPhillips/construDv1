...
//############################### 
	...
	CSD.rh.find_selected_parts_of_discussion_elements = function (id_of_mousedown_element) {
		var mousedown_div = CSD.model.get_element_by_id(id_of_mousedown_element);
		//check the node that was initially moused down in is a node and not a connection
		if (mousedown_div.is_a_node_q()) {
			var $mousedown_div = $('[element_id="' + id_of_mousedown_element + '"]');
			...
			var $mousedown_inner_element_div = $mousedown_div.children('.element_inner');
			var the_mousedown_div = $mousedown_inner_element_div.get(0);
			...
			var clicked_div_text = $mousedown_inner_element_div.text();
			...
			if (length_of_raw_selection > 0) {
				...
				if ((selected_text_string.indexOf_v2(part_of_div_text_to_test) === 0) && (part_of_div_text_to_test !== '')) {
					...
					if (selected_text_string.lastIndexOf_v2(part_of_div_text_to_test) === (length_of_raw_selection - part_of_div_text_to_test.length)) {
						...
						//  but we'll now avoid taking text backwards from the start point in $mousedown_inner_element_div, which would be nonsense.
						direction_of_selection_is_forward = true;
			...
			//user moused down in a none node so ignore it.
			return false;
		}
